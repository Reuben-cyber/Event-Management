...
&& in_array($extension, $allowedExts))
  ...
  if ($_FILES["file"]["error"] > 0)
    ...
    if (file_exists("upload/" . $_FILES["file"]["name"]))
      {
      $msg=$msg."Since the files already exists rename the file on your comp and upload it using Update Event Report option on the left.";
      }
      else {
  ...
  {
  $msg="Invalid file because you have not selected an image file at this time. Use Edit Report option from the menu and update the photo of this event.";
  }
//Logic for file upload endend
echo $msg."<br>";
$file_name=$_FILES["file"]["name"];
$pqr="SELECT COUNT(name) AS cold FROM all_events WHERE name='$name' AND YEAR(sdate)='$sdate'";
$result=mysqli_query($con,$pqr);
if(!$result) {die('error...'.mysqli_error($con));}
while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){$haha = $row["cold"];}
if($haha == 0)
{
	$sql= "INSERT INTO all_events(id, name, comments, Venue, theme, heads, sdate, edate, budget, numevents, events, numparticipants, footfall, winners, topic, speaker, numdelegates, numteams, chiefguests, service, teamleaders, ser_provided_to, commofficer, numsold, category, numbooks, supplier, genres, imagename) VALUES (NULL,'$name','$comments','$venue','$theme','$heads','$sdate','$edate','$budget','$numevents','$events','$numparticipants','$footfall','$winners','$topic','$speakers','$numdelegates','$numteams','$chiefguests','$service','$teamleaders','$ser_provided_to','$commofficer','$numsold','$category','$numbooks','$supplier','$genres','$file_name')";
...
{
		echo"Duplicate Report Not Allowed";		
}
