...
$options1[]="<option value=''></option>";
 ...
 $abc="SELECT DISTINCT name FROM all_events ORDER BY name";
  $result=mysqli_query($con,$abc);
 if(!$result){
	 die('records not found '.mysqli_errno($con));
 }
 while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
	 $options1[]="<option value='$row[name]'>$row[name]</option>	";
 }
mysqli_close($con);
...
$options2[]="<option value=''></option>";
 ...
 $bcd="SELECT DISTINCT YEAR(sdate) AS ysdate FROM all_events ORDER BY ysdate";
  $result=mysqli_query($con,$bcd);
 if(!$result){
	 die('records not found '.mysqli_errno($con));
 }
 while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
	 $options2[]="<option value='$row[ysdate]'>$row[ysdate]</option>";
 }
 mysqli_close($con);
 echo "<form method='post' action='updatereport.php'>";
 echo "Select Event: <select name='ename' size='1' REQUIRED>".implode('\n',$options1)."</select>";
