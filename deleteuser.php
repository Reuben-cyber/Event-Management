...
$options1[]="<option value=''></option>";
 ...
 $abc="SELECT DISTINCT Uname FROM user ORDER BY Uname";
  $result=mysqli_query($con,$abc);
 if(!$result){
	 die('records not found '.mysqli_errno($con));
 }
 while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
	 $options1[]="<option value='$row[Uname]'>$row[Uname]</option>	";
 }
mysqli_close($con);
 echo "<form method='post' action='deleteduser.php'>";
 echo "Select Username: <select name='Uname' size='1' REQUIRED>".implode('\n',$options1)."</select><br>";
