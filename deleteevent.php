...
$options1[]="<option value=''></option>";
 ...
 $abc="SELECT DISTINCT e_name FROM event_names ORDER BY e_name";
  $result=mysqli_query($con,$abc);
 if(!$result){
	 die('records not found '.mysqli_errno($con));
 }
 while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
	 $options1[]="<option value='$row[e_name]'>$row[e_name]</option>	";
 }
mysqli_close($con);
 echo "<form method='post' action='deletedevent.php'>";
 echo "Select Event: <select name='e_name' size='1' REQUIRED>".implode('\n',$options1)."</select><br>";
