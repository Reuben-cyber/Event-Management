...
$Uname=mysqli_real_escape_string($con,$Uname);
 ...
 $abc="SELECT * FROM user WHERE Name='$Uname'";
  $result=mysqli_query($con,$abc);
 if(!$result){
	 die('records not found '.mysqli_errno($con));
 }
 echo "<form method='post' action='updateduser.php'>";
 while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)){
	 
     echo "Name: <input type='hidden' name='name' value='$row[Name]'>$row[Name]<br>";
	 echo "User Id: <input type='text' name='username' value='$row[Uname]'><br>";
	 if($row['type'] == "ADMIN"){ $r="<option value='ADMIN' selected='selected'>ADMIN</option><option value='USER'>USER</option>"; } else { $r="<option value='ADMIN'>ADMIN</option><option value='USER' selected='selected'>USER</option>"; }
	 echo "Type of User: <select name='type' value='$row[type]'><br>";
	 echo "Password: <input type='password' name='pass' value='$row[pass]'><br>";
